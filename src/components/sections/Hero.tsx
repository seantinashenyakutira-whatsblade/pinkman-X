...
export default function Hero() {
  ...
  return (
    <section className="relative min-h-screen flex items-center pt-24 pb-16 px-4 overflow-hidden">
      ...
      <div className="absolute top-1/3 left-1/2 -translate-x-1/2 w-[900px] h-[900px] bg-gold/4 rounded-full blur-[140px] pointer-events-none" />
      <div className="max-w-6xl mx-auto w-full grid lg:grid-cols-2 gap-12 lg:gap-16 items-center relative z-10">
        <div className="text-center lg:text-left">
          ...
          <h1 className="text-4xl sm:text-5xl lg:text-6xl xl:text-7xl font-black tracking-tight leading-[1.05] mb-6">
            ...
            <br />
            <span className="bg-gradient-to-r from-gold to-amber bg-clip-text text-transparent">Intelligence</span>
          </h1>
          <p className="text-base sm:text-lg text-muted-light/70 max-w-xl mx-auto lg:mx-0 mb-8 leading-relaxed">
          ...
          <p className="text-xs text-muted italic">First 10 waitlist members receive 2 months of Pinkman X Pro free at launch.</p>
        </div>

        <div className="w-full max-w-md mx-auto lg:mx-0 lg:ml-auto">
          <div className="rounded-2xl bg-dark-800/80 border border-border/60 p-6 sm:p-8 backdrop-blur-sm">
            {status === 'success' ? (
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <input value={f.full_name} onChange={(e) => setF({ ...f, full_name: e.target.value })} placeholder="Full Name" className="w-full px-4 py-2.5 rounded-lg bg-dark-900 border border-border text-white text-sm placeholder-muted/60 focus:outline-none focus:ring-2 focus:ring-gold/25 focus:border-gold/40 transition-all" />
                <input value={f.email} onChange={(e) => setF({ ...f, email: e.target.value })} type="email" placeholder="Email Address" className="w-full px-4 py-2.5 rounded-lg bg-dark-900 border border-border text-white text-sm placeholder-muted/60 focus:outline-none focus:ring-2 focus:ring-gold/25 focus:border-gold/40 transition-all" />
                <input value={f.whatsapp} onChange={(e) => setF({ ...f, whatsapp: e.target.value })} placeholder="WhatsApp Number" className="w-full px-4 py-2.5 rounded-lg bg-dark-900 border border-border text-white text-sm placeholder-muted/60 focus:outline-none focus:ring-2 focus:ring-gold/25 focus:border-gold/40 transition-all" />
                <div className="grid grid-cols-2 gap-3">
                  <select value={f.experience_level} onChange={(e) => setF({ ...f, experience_level: e.target.value })} className="w-full px-3 py-2.5 rounded-lg bg-dark-900 border border-border text-white text-sm focus:outline-none focus:ring-2 focus:ring-gold/25 focus:border-gold/40">
                    <option value="">Experience</option>
                    {expOptions.map((o) => <option key={o} value={o}>{o}</option>)}
                  </select>
                  <select value={f.interest} onChange={(e) => setF({ ...f, interest: e.target.value })} className="w-full px-3 py-2.5 rounded-lg bg-dark-900 border border-border text-white text-sm focus:outline-none focus:ring-2 focus:ring-gold/25 focus:border-gold/40">
                ...
                <label className="flex items-start gap-2.5 cursor-pointer">
                  ...
                  <span className="text-xs text-muted-light">I agree to receive product updates, early access offers, and trading insights.</span>
                </label>
