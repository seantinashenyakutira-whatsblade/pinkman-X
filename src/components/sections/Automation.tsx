      ...
      <div className="max-w-6xl mx-auto grid lg:grid-cols-2 gap-12 items-center">
        <div>
          ...
          <h2 className="text-3xl sm:text-4xl font-bold mt-3 mb-4">
            Automation When{' '}
            <span className="bg-gradient-to-r from-gold to-amber bg-clip-text text-transparent">You're Ready</span>
          </h2>
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            {['Scalping', 'Day Trading', 'Swing Trading', 'Multi-Timeframe'].map((t) => (
              <span key={t} className="px-3 py-1.5 rounded-lg text-[10px] font-semibold bg-gold/5 text-gold border border-gold/15 uppercase tracking-wider">{t}</span>
            ))}
          </div>
        ...
        <div>
          <Card className="p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="px-2 py-0.5 rounded text-[10px] font-semibold bg-neon/10 text-neon border border-neon/20">ACTIVE</span>
            </div>
            <div className="h-24 mb-4 relative">
              <svg className="w-full h-full" viewBox="0 0 200 60" preserveAspectRatio="none">
                ...
                <path d="M0 50 Q10 48 20 45 Q30 42 40 35 Q50 28 60 30 Q70 32 80 25 Q90 18 100 20 Q110 22 120 15 Q130 8 140 12 Q150 16 160 10 Q170 4 180 8 Q190 12 200 5 L200 60 L0 60 Z" fill="url(#chartUp)" opacity="0.15" />
                <defs><linearGradient id="chartUp" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#D4AF37" /><stop offset="100%" stopColor="#D4AF37" stopOpacity="0" /></linearGradient></defs>
              </svg>
            </div>
            <div className="grid grid-cols-3 gap-3 text-center">
              {[{ label: 'Equity', val: '$12,450' }, { label: 'Drawdown', val: '3.2%' }, { label: 'Win Rate', val: '67%' }].map((s) => (
                <div key={s.label} className="py-2 rounded-lg bg-dark-900/50">
