...
export default function Chatbot() {
  ...
  const getBounds = useCallback(() => {
    ...
    if (open) {
      return {
        minX: pad,
        maxX: window.innerWidth - chatW - pad,
        minY: pad,
        maxY: window.innerHeight - chatH - pad,
      }
    }
    return {
      minX: pad,
      maxX: window.innerWidth - btnSize - pad,
      minY: pad,
      maxY: window.innerHeight - btnSize - pad,
    }
  }, [open])
  ...
  useEffect(() => {
    ...
    const onMove = (clientX: number, clientY: number) => {
      ...
      const nx = clamp(dragStart.current.elX + dx, b.minX, b.maxX)
      const ny = clamp(dragStart.current.elY + dy, b.minY, b.maxY)
  ...
  // Re-clamp position when open/close changes to keep within new bounds
  useEffect(() => {
    const b = getBounds()
    const nx = clamp(posRef.current.x, b.minX, b.maxX)
    const ny = clamp(posRef.current.y, b.minY, b.maxY)
    posRef.current = { x: nx, y: ny }
    setPos({ x: nx, y: ny })
  }, [open, clamp, getBounds])
  ...
  // Maximized = full viewport overlay
  if (maximized && open) {
    return (
      <div className="fixed inset-0 z-[60] flex flex-col bg-dark-950/95 backdrop-blur-2xl animate-fade-in">
        {/* Header */}
        <div className="flex items-center justify-between px-4 sm:px-6 py-3 border-b border-gold/10 shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-gold/10 border border-gold/20 flex items-center justify-center">
              <Bot className="w-5 h-5 text-gold" />
            </div>
            <div>
              <p className="text-base font-bold text-white font-sans">Pinkman X AI</p>
              <p className="text-xs text-emerald-400 font-sans">Online</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <span className="text-[10px] text-muted font-sans hidden sm:block">Drag to move</span>
            <button onClick={toggleMaximize} className="w-8 h-8 rounded-xl bg-black/60 border border-gold/10 flex items-center justify-center hover:bg-white/5 transition-colors">
              <Minimize2 className="w-4 h-4 text-muted-light" />
            </button>
            <button onClick={close} className="w-8 h-8 rounded-xl bg-black/60 border border-gold/10 flex items-center justify-center hover:bg-white/5 transition-colors">
        ...
        <div className="flex-1 overflow-y-auto px-4 sm:px-6 py-4">
          {messages.length === 0 && (
            <div className="flex flex-col items-center justify-center h-full text-center">
              <Bot className="w-14 h-14 text-gold/20 mx-auto mb-4" />
              <p className="text-lg text-muted-light font-medium font-sans mb-1">Ask me anything about Pinkman X!</p>
              <p className="text-sm text-muted font-sans mb-8">Pricing, features, launch date, anything.</p>
          ...
          <div className="max-w-3xl mx-auto space-y-4">
            {messages.map((m, i) => (
              <div key={i} className={`flex items-start gap-3 ${m.role === 'user' ? 'flex-row-reverse' : ''}`}>
                <div className={`w-8 h-8 rounded-xl shrink-0 flex items-center justify-center mt-0.5 ${m.role === 'user' ? 'bg-gold/10 border border-gold/20' : 'bg-amber/10 border border-amber/20'}`}>
                  {m.role === 'user' ? <User className="w-4 h-4 text-gold" /> : <Bot className="w-4 h-4 text-amber" />}
                </div>
                <div className={`max-w-[80%] sm:max-w-[70%] px-4 py-3 rounded-2xl ${m.role === 'user' ? 'bg-gold/10 border border-gold/20' : 'bg-black/60 border border-gold/10'}`}>
                  {m.role === 'user' ? (
            ...
            {loading && (
              <div className="flex items-start gap-3">
                <div className="w-8 h-8 rounded-xl bg-amber/10 border border-amber/20 flex items-center justify-center shrink-0">
                  <Bot className="w-4 h-4 text-amber" />
                </div>
                <div className="px-4 py-3 rounded-2xl bg-black/60 border border-gold/10">
        ...
        {/* Input */}
        <div className="border-t border-gold/10 shrink-0 px-4 sm:px-6 py-3">
          <form onSubmit={handleSend} className="max-w-3xl mx-auto flex items-center gap-3">
            <input
              value={input}
              onChange={(e) => setInput(e.target.value)}
              placeholder="Ask about Pinkman X..."
              className="flex-1 px-4 py-3 rounded-xl bg-black/60 border border-gold/15 text-white text-sm placeholder-muted/60 focus:outline-none focus:ring-2 focus:ring-gold/25 focus:border-gold/40 transition-all font-sans"
            />
            <button
              type="submit"
              disabled={loading || !input.trim()}
              className="w-10 h-10 rounded-xl bg-gradient-to-r from-gold to-amber text-dark-950 flex items-center justify-center hover:brightness-110 transition-all disabled:opacity-40 shrink-0"
            >
  ...
  return (
    ...
    >
      ...
      {/* Chat window */}
      {open && (
        <div className="rounded-2xl bg-black/80 backdrop-blur-xl border border-gold/20 shadow-[0_0_40px_rgba(212,175,55,0.1)] flex flex-col overflow-hidden animate-fade-in mb-3 transition-all duration-300 w-[340px] sm:w-[380px] h-[480px]">
          <div className="flex items-center justify-between px-4 py-3 border-b border-gold/10 shrink-0">
            ...
            <div className="flex items-center gap-1 chat-ignore-drag">
              ...
              <button onClick={toggleMaximize} className="w-7 h-7 rounded-lg bg-black/60 border border-gold/10 flex items-center justify-center hover:bg-white/5 transition-colors" title="Expand to fullscreen">
                <Maximize2 className="w-4 h-4 text-muted-light" />
              </button>
              <button onClick={close} className="w-7 h-7 rounded-lg bg-black/60 border border-gold/10 flex items-center justify-center hover:bg-white/5 transition-colors">
                <X className="w-4 h-4 text-muted-light" />
              </button>
            </div>
          </div>

          <div className="flex-1 overflow-y-auto p-4 chat-ignore-drag">
            {messages.length === 0 && (
              <div className="text-center py-4">
                ...
                <p className="text-xs text-muted mt-1 font-sans mb-4">Pricing, features, launch date, anything.</p>
